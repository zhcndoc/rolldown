...
- **JS API 与 Rollup 保持一致** — TypeScript 层的表现（事件、选项、插件钩子、生命周期顺序）应当与 Rollup 的行为一致，除非存在技术原因不允许。任何差异都会被明确记录。
- **close 可重入** — 监听器事件监听函数可以调用并等待 `watcher.close()`。`close` 的 Promise 仍然表示清理已完成；协调器不能通过仅仅把请求入队后就解析它来规避死锁。
- **Rust 代码遵循 Rust 习惯** — Rust 核心应当具有原生感：由所有权驱动、使用枚举状态机、基于 trait 的可扩展性，除架构所需之外不应有不必要的 `Arc`/`Mutex`。
- **整个栈保持一致的命名** — Rollup 定义了规范的事件/概念名称（例如 `BUNDLE_START`/`BUNDLE_END`）。Rust 侧应使用相同术语，以便在 NAPI 边界形成清晰的一一映射，避免在脑中做额外翻译。
