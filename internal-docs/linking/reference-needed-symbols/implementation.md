...
- **`Import`, `WrapKind::None`, not reexport** — 无记录；两侧都是扁平 ESM，没有包装器可调用。后续阶段仍然必须检查活动导入符号的规范所有者：一个未包装的 barrel 可以转发来自一个已包装 ESM 模块的绑定，而当 barrel 不在同一个 chunk 中执行时，该所有者在读取绑定之前仍需要调用其 `init_*()`；但这只在包装器可从 importer 的 chunk 到达时才成立（即在该 chunk 中已声明，或作为跨 chunk 导入被注册）；否则访问会通过 barrel 的命名空间流转，并由 barrel 所在 chunk 自行执行 init。
...
- [determine-module-exports-kind](../determine-module-exports-kind/implementation.md) — 生成 `wrap_kind` 和 `safely_merge_cjs_ns_map`。
- [module-execution-order](../module-execution-order/implementation.md) — 正交；`exec_order` 是 `include_statements` 用于确定性遍历模块的依据。
- `crates/rolldown/src/stages/link_stage/wrapping.rs` — 填充 `wrap_kind` 和 `wrapper_ref`。
- `crates/rolldown/src/stages/link_stage/tree_shaking/include_statements.rs` — `referenced_symbols`、`side_effect` 和 `depended_runtime_helper` 的消费者。
