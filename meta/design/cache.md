...
双层模型（bundler 级别 vs bundle 级别）记录在
[bundler-data-lifecycle.md](./bundler-data-lifecycle.md) 中；该文档也说明了构建失败时的缓存完整性。
...
一次构建会通过若干非原子性的“撕裂 → 修复”步骤来修改 `ScanStageCache`；在撕裂与修复之间过早返回的 `?` 可能会让缓存处于损坏状态，从而影响下一次构建。该不变量、三个被撕开的窗口（所有权 / 作用域 / defer-sync），以及“无条件修复”规则，都记录在 [bundler-data-lifecycle.md](./bundler-data-lifecycle.md)（“失败构建时的缓存完整性”）中。三个修复位置——`with_cached_bundle`、`bundle_up` 中 `merge_immutable_fields_for_cache` 的顺序，以及 `update_defer_sync_data`——都引用了该部分。

## 未解决的问题

- `merge` 中的 `module_id_to_idx[new_module.id()]` 索引在键缺失时会 panic，并且只会在内部不一致时触发；`Module::idx()` 可直接得到相同的值，而无需可能失败的查找。是否切换仍是一个跟进事项（需要审计，确认没有调用方把一个 `.idx` 不是由 loader 分配的 `Module` 传给 `merge`）。
- `merge` 是一个涉及多个字段的大型突变操作，中途不会有 `?`；但如果在 `merge` 过程中发生 panic（上面的两个位置），快照会仍然存在，但内部不一致。恢复存在性并不能保证一致性。

## 相关

- [bundler-data-lifecycle](./bundler-data-lifecycle.md) — bundler 级别与 bundle 级别的数据，`BundleMode`，失败构建时的缓存完整性。
- [module-id](./module-id.md) — `ModuleId` 设计。
- [rust-bundler](./rust-bundler.md) — `Bundler` 结构体与构建生命周期。
- [watch-mode](./watch-mode.md) — watch 模式，它会驱动部分扫描。
