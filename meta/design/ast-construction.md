...
每一种选择都受以下两个事实约束，这两个事实已在 [ast-mutation](./ast-mutation.md) 中记载：合成节点必须携带合成的 span（保留的 `SPAN`，`0..0`），这样它们才不会与以 span 为键的辅助表误匹配（见 `crates/rolldown/src/module_finalizers/mod.rs:1088`）；并且 rolldown 在 finalize 后不会重新运行语义分析，因此合成节点在其生命周期内都保持一个 dummy `NodeId`，而不会被回填。
...
- [ast-mutation](./ast-mutation.md) — 以 span/`NodeId` 作为身份标识的契约，它约束了合成节点
- [runtime-helpers](./runtime-helpers.md) — `make_*` 互操作构造函数所调用的运行时函数
